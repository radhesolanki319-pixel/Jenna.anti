...
const PAIR_PHONE = getArg('phone', process.env.WHATSAPP_PHONE || '');
...
async function startSocket() {
  ...
  if (PAIR_PHONE && !sock.authState.creds.registered) {
    setTimeout(async () => {
      try {
        const cleanNumber = PAIR_PHONE.replace(/[^0-9]/g, '');
        ...
        console.log(`   Enter code: ${formattedCode} (or ${code})\n`);
      } catch (err) {
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (qr && !PAIR_PHONE) {
      if (PAIR_JSON) {
        ...
        console.log('\nWaiting for scan...\n');
        try {
          const qrPngPath = '/storage/emulated/0/Download/jenna_whatsapp_qr.png';
          QRCode.toFile(qrPngPath, qr, { scale: 8, margin: 2 });
          console.log(`🖼️  QR Code saved as image to: ${qrPngPath}`);
          QRCode.toString(qr, { type: 'svg' }).then(svg => {
            const html = `<!DOCTYPE html><html><head><meta http-equiv="refresh" content="15"><title>Jenna AI WhatsApp QR</title><style>body{display:flex;flex-direction:column;align-items:center;justify-content:center;height:100vh;margin:0;background:#0f172a;color:#fff;font-family:system-ui,-apple-system,sans-serif}.card{background:#fff;padding:24px;border-radius:16px;box-shadow:0 10px 25px rgba(0,0,0,0.5)}h2{margin-bottom:8px;color:#38bdf8}p{color:#94a3b8;font-size:14px}</style></head><body><h2>Jenna AI WhatsApp QR</h2><p>WhatsApp Settings &gt; Linked Devices &gt; Link a Device</p><div class="card">${svg}</div><p>Auto-refreshes every 15s • Point your phone camera here</p></body></html>`;
            writeFileSync('/storage/emulated/0/Download/jenna_qr.html', html);
          });
        } catch (err) {
