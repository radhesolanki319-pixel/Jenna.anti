...
- THE 5-LAYER UNIVERSAL PROBLEM-SOLVING HIERARCHY (NEVER SAY 'MERE PAAS TOOL NAHI HAI'):
  Boss does NOT want you to rely on micro-tools for every specific action. You are a sovereign entity capable of handling ANY big or small task using these 5 universal layers:
  1. Direct Tool: If an exact tool exists (like get_weather, get_time, memory_recall, phone_vitals), use it in 1 step.
  2. Universal Screen Operator (Eyes + Hands): If Boss asks to do something in ANY app (WhatsApp, YouTube, Instagram, Phone Settings, Zomato, Gallery, Chrome) and there is no direct API, DO NOT FREEZE OR COMPLAIN! Use `universal_screen_operator` with Boss's goal. You look at the phone screen, see the buttons/text with vision, and tap/type/scroll like a human.
  3. Universal Script Runtime (Data & Logic): If Boss asks for data processing, scraping, calculations, or file conversions, write and run Python/Node scripts in the background silently. Extract the result and explain in sweet, clean Hinglish.
  4. Autonomous Skill Synthesis (Remembering Workflows): When you solve a new multi-step task, save it as a reusable skill via `learn_skill` so you master it forever.
  5. Multi-Agent Swarms (Large Projects): For massive tasks (building an entire app, writing a full website, large refactors), dispatch specialized subagents (`invoke_subagent`) in parallel.
...
class AntigravityAgent:
    ...
    async def execute_tool(self, tool_name: str, args: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            # 81. whatsapp_groups
            elif tool_name in ("whatsapp_groups", "get_groups", "list_groups"):
                import httpx
                for url in ("http://127.0.0.1:3000/groups", "https://jenna-anti.onrender.com/whatsapp/groups"):
                    try:
                        async with httpx.AsyncClient(timeout=8.0) as client:
                            ...
                            if r.status_code == 200:
                                d = r.json()
                                if d.get("success"):
                                    return {"tool": "whatsapp_groups", **d}
                    except Exception:
                        pass
                return {"tool": "whatsapp_groups", "success": False, "groups": [], "message": "WhatsApp bridge groups currently unavailable."}

            # 82. universal_screen_operator
            elif tool_name in ("universal_screen_operator", "screen_goal", "solve_on_screen"):
                from app.services.screen_vision import execute_goal
                goal_text = args.get("goal") or args.get("task", "")
                max_s = int(args.get("max_steps", 12))
                res = await execute_goal(goal_text, max_steps=max_s)
                return {
                    "tool": "universal_screen_operator",
                    "goal": goal_text,
                    "achieved": res.get("goal_completed", False),
                    "steps_taken": len(res.get("steps", [])),
                    "summary": f"Executed goal '{goal_text}' across {len(res.get('steps', []))} visual steps.",
                    "success": True,
                }
    ...
    async def run_agent_loop(
        ...
        while iteration < max_iterations:
            ...
            for tc in tool_calls:
                ...
                # Yield user-visible stream deltas for rich live feedback (NEVER output raw bash code blocks)
                if tool_name == "run_command":
                    yield {
                        "type": "stream.delta",
                        "delta": "> ⚡ *Action performed in background*\n\n",
                    }
